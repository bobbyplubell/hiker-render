...
fn draw(packet: &Packet, opts: &MermaidOptions) -> MermaidRender {
    ...
    // Set of bit positions that begin some segment, keyed by row. Used to
    // suppress a segment's end-bit ruler label when the very next bit starts an
    // adjacent segment in the same row: otherwise `end` (anchor=end) and the
    // next segment's `start` (anchor=start) land on the same x and overprint
    // (e.g. "15" + "16" reads as "1516"). Mermaid only labels each field's
    // start at its left edge plus the final end, so we drop the redundant
    // shared boundary number.
    use std::collections::HashSet;
    let starts: HashSet<(usize, usize)> =
        segs.iter().map(|s| (s.row, s.start)).collect();
    ...
    for seg in &segs {
        ...
        // Draw the end-bit label unless the next bit begins an adjacent segment
        // in this row (its start label would overprint our end label).
        let next_is_adjacent = starts.contains(&(seg.row, seg.end + 1));
        if !single && !next_is_adjacent {
            let _ = write!(
                body,
                "<text x=\"{:.2}\" y=\"{:.2}\" font-family=\"{}\" font-size=\"{:.2}\" \
...
mod tests {
    ...
    fn bit_ruler_numbers_present() {
        let src = "packet-beta\n0-15: \"A\"\n16-31: \"B\"\n";
        let r = render_packet(src, &opts()).unwrap();
        // Ruler shows each field's start at its left edge plus the final end:
        // 0, 16, 31 appear. The shared boundary end "15" is suppressed because
        // the next field starts at the adjacent bit 16 — drawing both would
        // overprint (anchor=end "15" + anchor=start "16" at the same x → "1516").
        for n in ["0", "16", "31"] {
            assert!(r.svg.contains(&format!(">{n}<")), "missing ruler index {n}");
        }
        assert!(
            !r.svg.contains(">15<"),
            "redundant shared-boundary end label 15 should be suppressed"
        );
    }
