...
struct Layout {
    ...
    height: f32,
    title_h: f32,
    /// Width of the left lane-label gutter (computed to fit the widest label).
    label_w: f32,
...
const MIN_LABEL_W: f32 = 150.0; // minimum left gutter for lane labels
...
/// Width of the left lane-label gutter: wide enough that the longest lane label
/// (drawn bold, centered at `label_w/2`) fits without spilling past the left
/// edge. Lane labels are bold, so measure a bit wider than the plain advance.
fn label_gutter_w(fs: f32) -> f32 {
    let widest = LANES
        .iter()
        .map(|l| text_size(l, fs).0)
        .fold(0.0_f32, f32::max);
    // Bold weight + a little side padding on each side of the centered label.
    (widest * 1.08 + 2.0 * PAD).max(MIN_LABEL_W)
}

fn layout(model: &Model, opts: &MermaidOptions) -> Layout {
    let fs = opts.font_size_px;
    let label_w = label_gutter_w(fs);
    let content_x0 = label_w + PAD;
    ...
    Layout { width, height, title_h, label_w, bands, cards, conns }
}
