...
fn emit_cluster(svg: &mut String, cluster: &PositionedCluster, opts: &MermaidOptions) {
    ...
    // Cluster fill (mermaid's clusterBkg). If it's opaque we apply a faint
    // ~0.3 opacity so the box reads as a backdrop without overpowering the
    // nodes inside it; if it already carries alpha (< 255) we respect that.
    let mut fill = opts.cluster_fill;
    if fill[3] == 255 {
        fill[3] = 77; // ~0.3 opacity
    }
    let (fill_s, fo) = fill_attrs(fill);
    let (stroke, so) = stroke_attrs(opts.cluster_stroke);
...
fn emit_edge_label(svg: &mut String, edge: &PositionedEdge, opts: &MermaidOptions) {
    ...
    // Backing so the label reads over the edge line; only painted when the
    // edge-label-bg color is opaque (a transparent canvas paints no box).
...
fn emit_node(svg: &mut String, node: &PositionedNode, opts: &MermaidOptions) {
    ...
    // `opacity:` in a classDef/style fades the whole shape (fill + stroke).
    let op = crate::svgutil::element_opacity_attr(node.style.opacity);
    let style = format!("fill=\"{fill}\"{fo} stroke=\"{stroke}\"{so} stroke-width=\"{sw}\"{op}");
    ...
    // Per-node label text properties (font-weight/font-style/text-decoration)
    // fall back to none; font-size falls back to the theme default.
    let font_size = node.style.font_size.unwrap_or(opts.font_size_px);
    let extra_attrs = crate::svgutil::text_style_attrs(&node.style);
    // Rich label: markdown emphasis, `<br>`/`\n` lines, and inline `$…$` math.
    // Plain labels render as a single centered `<text>`, identical to before.
    crate::label::emit_with_attrs(
        svg,
        label,
        cx,
        cy,
...
mod tests {
    ...
    #[test]
    fn edge_label_bg_uses_edge_label_bg_color() {
        let mut opts = MermaidOptions::default();
        opts.edge_label_bg = [10, 20, 30, 255];
        let svg = draw_svg(&small_diagram(), &opts);
        // The edge-label backing rect uses the (opaque) edge-label-bg color.
        assert!(svg.contains("fill=\"rgb(10,20,30)\""), "got: {svg}");
        assert!(!svg.contains("fill-opacity=\"0.85\""));
    }

    #[test]
    fn edge_label_has_no_box_when_label_bg_transparent() {
        // A transparent edge-label-bg (e.g. the app's transparent canvas) must
        // NOT paint a box behind the label — that was the opaque-black-box bug.
        let mut opts = MermaidOptions::default();
        opts.edge_label_bg = [0, 0, 0, 0];
        let svg = draw_svg(&small_diagram(), &opts);
        assert!(
            !svg.contains("fill=\"rgb(0,0,0)\""),
            "transparent label-bg must not paint a black box: {svg}"
        );
    }
    ...
    #[test]
    fn cluster_uses_cluster_colors_not_node_colors() {
        // The cluster box must pick up opts.cluster_fill / cluster_stroke, not
        // the node colors. Use distinctive values to tell them apart.
        let mut d = small_diagram();
        d.clusters = vec![PositionedCluster {
            title: "G".to_string(),
            x: 5.0,
        ...
        let mut opts = MermaidOptions::default();
        opts.cluster_fill = [12, 34, 56, 255];
        opts.cluster_stroke = [200, 100, 50, 255];
        let svg = draw_svg(&d, &opts);
        // Cluster fill is opaque → painted at the faint ~0.3 tint opacity.
        assert!(svg.contains("fill=\"rgb(12,34,56)\""), "got: {svg}");
        assert!(svg.contains("fill-opacity=\"0.3"), "faint tint expected: {svg}");
        assert!(svg.contains("stroke=\"rgb(200,100,50)\""), "got: {svg}");
    }

    #[test]
    fn cluster_respects_preexisting_alpha() {
        // A cluster_fill already carrying alpha is used as-is (no 0.3 override).
        let mut d = small_diagram();
        d.clusters = vec![PositionedCluster {
            title: String::new(),
            x: 5.0,
        ...
        let mut opts = MermaidOptions::default();
        opts.cluster_fill = [12, 34, 56, 128]; // ~0.502
        let svg = draw_svg(&d, &opts);
        assert!(svg.contains("fill=\"rgb(12,34,56)\""), "got: {svg}");
        assert!(svg.contains("fill-opacity=\"0.5"), "alpha respected: {svg}");
        assert!(!svg.contains("fill-opacity=\"0.3"), "must not force 0.3: {svg}");
    }

    #[test]
    fn node_opacity_and_font_props_emitted() {
        use crate::model::ElemStyle;
        let mut d = small_diagram();
        d.nodes[0].style = ElemStyle {
            opacity: Some(0.4),
        ...
        let svg = draw_svg(&d, &MermaidOptions::default());
        // Opacity on the node shape.
        assert!(svg.contains("opacity=\"0.4"), "node opacity missing: {svg}");
        // Font props on the node label <text>.
        assert!(svg.contains("font-weight=\"bold\""), "got: {svg}");
        assert!(svg.contains("font-style=\"italic\""), "got: {svg}");
        assert!(svg.contains("text-decoration=\"underline\""), "got: {svg}");
        // Overridden font-size used for the label.
        assert!(svg.contains("font-size=\"22\""), "font-size override missing: {svg}");
    }
