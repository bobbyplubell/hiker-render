...
//!   Title (falling back to the id). The `(icon)` name selects a small
//!   monochrome vector glyph drawn above the title (`cloud`, `database`,
//!   `server`, `disk`, `internet`; any unknown name falls back to a generic box
//!   glyph).
...
//!
//! Simplifications (NOTE): no force relaxation, no
...
/// Drawn size (px) of a service icon glyph (square), and the gap between the
/// icon and the title below it.
...
pub fn render_architecture(src: &str, opts: &MermaidOptions) -> Result<MermaidRender, MermaidError> {
    ...
    let sizes: Vec<(f32, f32)> = diag
        ...
        .map(|s| {
            ...
            let mut h = th;
            if s.icon.is_some() {
                // Reserve a square icon glyph above the title plus a small gap.
                w = w.max(ICON_SIZE);
                h += ICON_SIZE + ICON_GAP;
            }
    ...
    for (i, s) in diag.services.iter().enumerate() {
        ...
        // Icon glyph on top (if any), with the title centered below it.
        let has_icon = s.icon.is_some();
        if let Some(icon) = &s.icon {
            // Icon occupies a square ICON_SIZE region centered horizontally,
            // sitting at the top of the box just inside the padding.
            let ix = p.x + p.w / 2.0 - ICON_SIZE / 2.0;
            let iy = p.y + opts.node_padding_y;
            draw_icon(&mut svg, icon, ix, iy, ICON_SIZE, opts);
        }
        // Title: centered vertically when there is no icon; just below the icon
        // glyph otherwise (icon on top, label below — as mermaid lays it out).
        let title_y = if has_icon {
            p.y + opts.node_padding_y + ICON_SIZE + ICON_GAP + fs * 0.8
        } else {
            p.y + p.h / 2.0 + fs * 0.32
        };
        let _ = write!(
...
/// Draw a small monochrome vector icon for `name` into the square region at
/// (`ox`, `oy`) of side `sz`. Icons use the node stroke colour. Coordinates in
/// each arm below are expressed in a 0..1 unit box and mapped onto the region by
/// the closures `px`/`py`; this keeps the path data readable and resolution
/// independent. Unknown names fall back to a generic box glyph (never fails).
///
/// The emitted markup is wrapped in `<g class="archicon archicon-<name>">` so
/// the chosen glyph is identifiable in the output.
fn draw_icon(svg: &mut String, name: &str, ox: f32, oy: f32, sz: f32, opts: &MermaidOptions) {
    let stroke = rgb(opts.node_stroke);
    let so = opacity_attr("stroke-opacity", opts.node_stroke);
    // Map unit-box coords (0..1) to pixel coords within the icon region.
    let px = |u: f32| ox + u * sz;
    let py = |u: f32| oy + u * sz;
    // Stroke width scales gently with the icon size.
    let sw = (sz / 18.0).max(1.0);

    // Pick a canonical glyph for the requested name. We strip a leading
    // provider prefix (e.g. `logos:aws-...`) and keep only the trailing token so
    // common iconify-style names still map onto a sensible built-in.
    let key = name.rsplit(':').next().unwrap_or(name).to_ascii_lowercase();
    let kind = if key.contains("cloud") {
        "cloud"
    } else if key.contains("database") || key.contains("db") {
        "database"
    } else if key.contains("disk") || key.contains("storage") {
        "disk"
    } else if key.contains("internet") || key.contains("globe") || key.contains("web") {
        "internet"
    ...
    let _ = write!(svg, "<g class=\"archicon archicon-{kind}\" fill=\"none\" stroke=\"{stroke}\"{so} \
        stroke-width=\"{sw:.1}\" stroke-linejoin=\"round\" stroke-linecap=\"round\">");

    match kind {
        // Cloud: a lobed outline (three bumps over a flat base).
        "cloud" => {
            let _ = write!(
                svg,
                "<path d=\"M {x0:.1} {y0:.1} \
                 C {c1x:.1} {y0:.1} {c2x:.1} {ty:.1} {mx:.1} {ty:.1} \
                 C {c3x:.1} {ty:.1} {x1:.1} {y0:.1} {x1:.1} {y0:.1} \
                 Z\"/>",
                x0 = px(0.18),
                y0 = py(0.70), // base line
                c1x = px(0.10),
                c2x = px(0.22),
                ty = py(0.32), // top of the bumps
                mx = px(0.50),
                c3x = px(0.78),
                x1 = px(0.82),
            );
            ...
            let _ = write!(
                svg,
                "<path d=\"M {x0:.1} {y:.1} L {x1:.1} {y:.1}\"/>",
                x0 = px(0.18),
                x1 = px(0.82),
                y = py(0.70),
            );
        }
        // Database: a cylinder — top ellipse plus side walls and a bottom curve.
        "database" => {
            let (l, r, t, b) = (px(0.22), px(0.78), py(0.20), py(0.80));
            let ry = sz * 0.09; // ellipse vertical radius
            // Top ellipse.
            let _ = write!(
                svg,
                "<ellipse cx=\"{cx:.1}\" cy=\"{cy:.1}\" rx=\"{rx:.1}\" ry=\"{rry:.1}\"/>",
                cx = (l + r) / 2.0,
                cy = t,
                rx = (r - l) / 2.0,
                rry = ry,
            ...
            let _ = write!(svg, "<path d=\"M {l:.1} {t:.1} L {l:.1} {b:.1}\"/>");
            let _ = write!(svg, "<path d=\"M {r:.1} {t:.1} L {r:.1} {b:.1}\"/>");
            // Bottom curve closing the cylinder.
            let _ = write!(
                svg,
                "<path d=\"M {l:.1} {b:.1} A {rx:.1} {rry:.1} 0 0 0 {r:.1} {b:.1}\"/>",
                rx = (r - l) / 2.0,
                rry = ry,
            );
        }
        // Server: a stacked rack of two units with a status indicator dot each.
        "server" => {
            let (l, r) = (px(0.20), px(0.80));
            for (top, bot) in [(py(0.22), py(0.46)), (py(0.54), py(0.78))] {
                let _ = write!(
                    svg,
                    "<rect x=\"{l:.1}\" y=\"{top:.1}\" width=\"{w:.1}\" height=\"{h:.1}\" rx=\"2\"/>",
                    w = r - l,
                    h = bot - top,
                );
                // Status dot near the left edge of each unit.
                let _ = write!(
                    svg,
                    "<circle cx=\"{cx:.1}\" cy=\"{cy:.1}\" r=\"{rr:.1}\" fill=\"{stroke}\"{fo}/>",
                    cx = l + (r - l) * 0.12,
                    cy = (top + bot) / 2.0,
                    rr = sz * 0.03,
        ...
        // Disk: a hard-disk platter — outer circle with a small spindle hub.
        "disk" => {
            let cx = px(0.50);
            let cy = py(0.50);
            let _ = write!(
            ...
            let _ = write!(
                svg,
                "<circle cx=\"{cx:.1}\" cy=\"{cy:.1}\" r=\"{r:.1}\" fill=\"{stroke}\"{fo}/>",
                r = sz * 0.07,
                fo = opacity_attr("fill-opacity", opts.node_stroke),
            );
            // Read/write head arm reaching toward the centre.
            let _ = write!(
                svg,
                "<path d=\"M {x0:.1} {y0:.1} L {x1:.1} {y1:.1}\"/>",
                x0 = px(0.80),
                y0 = py(0.20),
                x1 = px(0.56),
                y1 = py(0.44),
        ...
        // Internet: a globe — circle with one meridian and two latitude lines.
        "internet" => {
            let cx = px(0.50);
            let cy = py(0.50);
            let r = sz * 0.30;
            let _ = write!(svg, "<circle cx=\"{cx:.1}\" cy=\"{cy:.1}\" r=\"{r:.1}\"/>");
            // Equator + one latitude band above and below.
            let _ = write!(
                svg,
                "<path d=\"M {l:.1} {cy:.1} L {rr:.1} {cy:.1}\"/>",
                l = cx - r,
                rr = cx + r,
            );
            // Vertical meridian (an ellipse drawn as two arcs).
            let _ = write!(
                svg,
            ...
            // Latitude lines (shortened horizontal chords).
            let _ = write!(
                svg,
                "<path d=\"M {l:.1} {y:.1} L {rr:.1} {y:.1}\"/>",
                l = cx - r * 0.78,
                rr = cx + r * 0.78,
                y = cy - r * 0.5,
            );
        ...
        // Generic fallback: a plain rounded box with a folded top-right corner.
        _ => {
            let (l, t, r, b) = (px(0.22), py(0.22), px(0.78), py(0.78));
            let _ = write!(
                svg,
                "<rect x=\"{l:.1}\" y=\"{t:.1}\" width=\"{w:.1}\" height=\"{h:.1}\" rx=\"3\"/>",
                w = r - l,
                h = b - t,
            );
...
mod tests {
    ...
    #[test]
    fn render_draws_icon_glyphs() {
        let src = "architecture-beta
            service db(database)[Database]
            service srv(server)[Server]
            service net(internet)[Web]
            service unk(mystery)[Other]
            db:R -- L:srv
        ";
        let r = render_architecture(src, &opts()).unwrap();
        // Each known icon emits its identifiable glyph group.
        assert!(r.svg.contains("archicon-database"), "database glyph drawn");
        assert!(r.svg.contains("archicon-server"), "server glyph drawn");
        assert!(r.svg.contains("archicon-internet"), "internet glyph drawn");
        // The database cylinder uses an <ellipse> for its top.
        assert!(r.svg.contains("<ellipse"), "database cylinder top");
        // Unknown icon names fall back to the generic glyph (never panics).
        assert!(r.svg.contains("archicon-generic"), "unknown icon → generic glyph");
    }
