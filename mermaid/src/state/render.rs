...
fn emit_node(
    ...
    match s.pseudo {
        ...
        None => {
            ...
            let stroke_c = s.style.stroke.unwrap_or(opts.node_stroke);
            let sw = s.style.stroke_width.unwrap_or(1.5);
            // `opacity:` fades the box; font props ride on the label `<text>`.
            let op = crate::svgutil::element_opacity_attr(s.style.opacity);
            let _ = write!(
                svg,
                "<rect x=\"{x:.2}\" y=\"{y:.2}\" width=\"{w:.2}\" height=\"{h:.2}\" \
                 rx=\"6\" ry=\"6\" fill=\"{fill}\"{fo} stroke=\"{stroke}\"{so} stroke-width=\"{sw}\"{op}/>",
                fill = rgb(fill_c),
                fo = opacity_attr("fill-opacity", fill_c),
                stroke = rgb(stroke_c),
                so = opacity_attr("stroke-opacity", stroke_c),
            ...
            let font_size = s.style.font_size.unwrap_or(opts.font_size_px);
            let text_attrs = crate::svgutil::text_style_attrs(&s.style);
