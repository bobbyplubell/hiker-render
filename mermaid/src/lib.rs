...
pub struct MermaidOptions {
    ...
    pub background: [u8; 4],
    /// Backing color for edge / relationship labels (mermaid's
    /// `edgeLabelBackground`) as straight RGBA, so a label reads over the line
    /// it sits on. A label box is only painted when this is not fully
    /// transparent (alpha > 0) — a transparent canvas (`background` alpha 0)
    /// otherwise has no surface color, and painting `background`'s RGB at fixed
    /// opacity produced an opaque (often black) box. Defaults to the theme
    /// background; the host can override it with an opaque surface color.
    pub edge_label_bg: [u8; 4],
    /// Node fill / stroke as straight RGBA.
    pub node_fill: [u8; 4],
    pub node_stroke: [u8; 4],
    /// Subgraph / cluster box fill (mermaid's `clusterBkg`) as straight RGBA.
    /// A light surface behind a grouping box. If opaque (alpha 255), the cluster
    /// renderer paints it at a faint opacity so the box reads as a backdrop; if
    /// it already carries alpha (< 255), that alpha is respected as-is. Set by
    /// the theme; overridable via `themeVariables.clusterBkg` (or `tertiaryColor`
    /// as a fallback).
    pub cluster_fill: [u8; 4],
    /// Subgraph / cluster box border (mermaid's `clusterBorder`) as straight
    /// RGBA. Set by the theme; overridable via `themeVariables.clusterBorder`.
...
fn preprocess_opts(src: &str, opts: &MermaidOptions) -> (String, Option<MermaidOptions>) {
        ...
        || cfg.theme_vars.any()
    {
        ...
        // `themeVariables` override the chosen/base theme, so apply them last.
        cfg.theme_vars.apply_to(&mut o);
        (clean, Some(o))
    } else {
        (clean, None)
    }
...
mod region_tests {
    ...
    #[test]
    fn theme_variables_only_triggers_override_clone() {
        // No theme/look/font — only themeVariables. The clone condition must
        // still fire and the override must land on the returned options.
        let base = MermaidOptions::default();
        let src = "---\nconfig:\n  themeVariables:\n    primaryColor: \"#ff0000\"\n---\ngraph TD\n  A --> B";
        let (clean, owned) = preprocess_opts(src, &base);
        let o = owned.expect("themeVariables-only should produce an owned clone");
        assert_eq!(o.node_fill, [255, 0, 0, 255]);
        assert!(clean.trim_start().starts_with("graph TD"));
    }
}
