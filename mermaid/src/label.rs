...
pub fn emit(
    ...
    emit_with_attrs(svg, label, cx, cy, anchor, font_size, color, font_family, "");
}

/// Like [`emit`], but appends `extra_attrs` (a pre-built run of SVG attribute
/// strings, e.g. ` font-weight="bold" text-decoration="underline"`) onto every
/// `<text>` element it produces. Used to honor per-element classDef/style text
/// properties (font-weight/font-style/text-decoration). `extra_attrs` is emitted
/// verbatim, so callers must supply already-escaped, leading-space-prefixed text.
...
pub fn emit_with_attrs(
    ...
    if !is_rich(label) {
        emit_plain(svg, label, cx, cy, anchor, font_size, color, font_family, extra_attrs);
        return;
    }
                    ...
                    if !s.is_empty() {
                        ...
                        let _ = write!(
                            ...
                            "<text x=\"{x:.2}\" y=\"{baseline_y:.2}\" text-anchor=\"start\" \
                             ...
                             fill=\"{fill}\"{fo}{weight}{fstyle}{extra_attrs}>{}</text>",
                            escape(s),
                        );
                    }
                    let w = font::line_width(s, font_size);
...
/// identical to the previous node-label rendering. `extra_attrs` is appended
/// verbatim onto the `<text>` element (see [`emit_with_attrs`]).
#[allow(clippy::too_many_arguments)]
fn emit_plain(
    svg: &mut String,
    label: &str,
    ...
    let _ = write!(
        ...
        "<text x=\"{x:.2}\" y=\"{cy:.2}\" text-anchor=\"{text_anchor}\" \
         ...
         font-size=\"{font_size}\" fill=\"{fill}\"{fo}{extra_attrs}>",
    );
    if lines.len() == 1 {
