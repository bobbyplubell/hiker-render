...
const GROUP_STEP: f32 = 16.0; // gutter step per group nesting level
const GROUP_PAD: f32 = 8.0; // gap between names and the innermost bracket
const GROUP_LABEL_PAD: f32 = 10.0; // gap from a bracket to its rotated label
...
struct Group {
    ...
    height: usize,
    name: Option<String>,
    /// Nesting depth (0 = outermost). Deeper groups draw nearer the names;
    /// shallower groups draw further left in the gutter.
    depth: usize,
}
...
struct RecState {
    ...
    xx: f32,
    name: Option<String>,
    /// Current nesting depth while walking (0 at the root array).
    depth: usize,
}
...
pub fn render(root: &Value, opts: &WaveDromOptions) -> Result<WaveDromRender, WaveDromError> {
    ...
    // expect it bracketed, so add it as a group spanning all lanes. It encloses
    // everything, so it is the outermost (depth 0) bracket: bump every other
    // group one level deeper so the root sits furthest left in the gutter.
    if let Some(name) = root_name {
        for g in &mut groups {
            g.depth += 1;
        }
        groups.push(Group { x: root_xx, y: 0, height: lanes.len(), name: Some(name), depth: 0 });
    }
    ...
    let mut name_text_max = 0.0f32; // widest plain name width (no indent)
    ...
    for ln in &lanes {
        ...
        let name_w = crate::font::line_width(&name, 11.0);
        name_text_max = name_text_max.max(name_w);
        let nw = name_w + ln.indent;
    ...
    // Group brackets/labels live in the LEFT name gutter (negative lane-space
    // x, left of the right-anchored signal names), nested deeper = nearer the
    // names, matching WaveDrom's `groupX` layout. Compute the placement here so
    // both the gutter reservation (`xg`) and `render_groups` agree.
    //
    // The innermost bracket (`depth == max_depth`) sits just left of the widest
    // name; each shallower level steps a further `GROUP_STEP` left. Each bracket
    // also reserves `GROUP_LABEL_PAD` to its left for the rotated label.
    let max_depth = groups.iter().map(|g| g.depth).max().unwrap_or(0);
    // Right edge of the bracket region = left edge of the names minus a pad.
    let bracket_x_inner = TGO - name_text_max - GROUP_PAD;
    // x of a group's bracket given its depth (more negative = further left).
    let group_bracket_x =
        |depth: usize| bracket_x_inner - (max_depth - depth) as f32 * GROUP_STEP;
    // Leftmost extent the gutter must cover (outermost bracket + its label).
    let gutter_left = if groups.is_empty() {
        0.0
    } else {
        -(group_bracket_x(0) - GROUP_LABEL_PAD)
    };

    // Left column width (xg): round the widest name up to an xs grid, like
    // render-signal.js (`ceil((xmax - tgo)/xs)*xs`), but never less than the
    // group gutter so brackets/labels are not clipped off the left edge.
    let xg = ((name_col - TGO) / XS).ceil().max(1.0) * XS;
    let xg = xg.max((gutter_left / XS).ceil().max(1.0) * XS);
    ...
    // Move each group's bracket into the negative-x gutter computed above
    // (overwriting the positive accumulated indent `rec` left in `g.x`).
    for g in &mut groups {
        g.x = group_bracket_x(g.depth);
    }
...
fn render_marks(
    ...
    // Head/foot text (centered over the wave area). Use the skin foreground so
    // it stays visible under dark/lowkey skins (hardcoded black vanished).
    ...
    if let Some(t) = root.get("head").and_then(|h| h.get("text")).and_then(|v| v.as_str()) {
        ...
        text(svg, t, mid, y, "middle", 16.0, "Liberation Sans", fg, Some("bold"));
    }
...
fn ticktock(
    ...
    // WaveDrom `every: N` strides the labels: draw only every Nth cycle
    // (starting from the offset, i.e. i == 0). `every` absent or <= 1 → label
    // every cycle (unchanged behavior).
    let every = root
        .get(anchor)
...
mod tests {
    ...
    /// Regression for the "group bracket/label drawn over the waveforms" bug:
    /// both the bracket path and the rotated label must sit at NEGATIVE
    /// lane-space x (the left name gutter), never on the positive brick area.
    #[test]
    fn group_bracket_and_label_are_in_left_gutter() {
        let v = parse(
            r#"{signal:["ctrl",
                [{name:"req",wave:"01.0"},{name:"ack",wave:"0.10"}],
                {name:"clk",wave:"p..."}]}"#,
        );
        let r = render(&v, &WaveDromOptions::default()).expect("ok");

        // Every group bracket path begins `m <x>,<y> c -3,0 ...` — x must be < 0.
        let mut bracket_count = 0;
        for cap in r.svg.split("<path d=\"m ").skip(1) {
            if !cap.contains("c -3,0 -5,2 -5,5") {
                continue; // not a group bracket
            }
            bracket_count += 1;
            let x: f32 = cap.split(',').next().unwrap().trim().parse().expect("bracket x");
            assert!(x < 0.0, "group bracket x must be in the left gutter, got {x}");
        }
        assert!(bracket_count >= 2, "expected nested + root brackets, got {bracket_count}");

        // The rotated group label is emitted as
        // `<g transform="translate(<x>,<y>)"><g transform="rotate(270)">`.
        let mut label_count = 0;
        for piece in r.svg.split("<g transform=\"translate(").skip(1) {
            let head = &piece[..piece.find(')').unwrap_or(piece.len())];
            let rest = &piece[piece.find(')').map(|i| i + 1).unwrap_or(piece.len())..];
            if !rest.starts_with("\"><g transform=\"rotate(270)") {
                continue; // not a group label wrapper
            }
            label_count += 1;
            let lx: f32 = head.split(',').next().unwrap().trim().parse().expect("label x");
            assert!(lx < 0.0, "group label x must be in the left gutter, got {lx}");
        }
        assert_eq!(label_count, 1, "expected the single named 'ctrl' label");
    }
    ...
    #[test]
    fn dark_skin_head_text_uses_visible_fg() {
        // Bug 2: head/foot text was hardcoded black → invisible on dark skin.
        // It must now use the skin foreground (#fff for dark).
        let v = parse(
            r#"{signal:[{name:"clk",wave:"p..."}],
               head:{text:"Title"}, config:{skin:"dark"}}"#,
        );
        let r = render(&v, &WaveDromOptions::default()).expect("ok");
        // The head text element must carry the light foreground, not black.
        let title = r.svg.split(">Title<").next().unwrap();
        let attrs = title.rsplit("<text").next().unwrap();
        assert!(attrs.contains("rgb(255,255,255)"), "dark head text should be light: {attrs}");
        assert!(!attrs.contains("rgb(0,0,0)"), "dark head text must not be black: {attrs}");
    }

    #[test]
    fn head_every_strides_tick_labels() {
        // Bug 3: `every: 2` should label only every other cycle → half as many
        // tick labels as the unstrided version.
        let base = parse(r#"{signal:[{name:"clk",wave:"p.......",period:1}], head:{tick:0}}"#);
        let strided =
            parse(r#"{signal:[{name:"clk",wave:"p.......",period:1}], head:{tick:0,every:2}}"#);
        let rb = render(&base, &WaveDromOptions::default()).expect("ok");
        let rs = render(&strided, &WaveDromOptions::default()).expect("ok");

        // Count tick number labels (head ticks are rendered as <text ...>N</text>
        // in the muted grid color). Counting <text> elements is noisy, so count
        // the cycle digits emitted by the head tick row via the dominant marker:
        // every base tick "0..N" appears; strided drops the odd ones.
        let count_ticks = |svg: &str| {
            // Head ticks sit at y="-5.00"; count those text elements.
            svg.matches("y=\"-5.00\"").count()
        };
        let nb = count_ticks(&rb.svg);
        let ns = count_ticks(&rs.svg);
        assert!(nb > 0, "base should have head ticks");
        assert_eq!(ns, nb.div_ceil(2), "every:2 should yield ~half the ticks ({nb} -> {ns})");
    }
