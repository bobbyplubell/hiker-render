...
fn read_matrix(
    ...
    let (kind, col_align): (MatrixKind, Vec<Align>) = match &events[*i] {
        Event::Begin(Grouping::Array(cols)) => {
            let aligns = cols
                .iter()
                .filter_map(|c| match c {
                    ArrayColumn::Column(a) => Some(map_align(*a)),
                    ArrayColumn::Separator(_) => None, // TODO: vertical rules
                })
                .collect();
            (MatrixKind::Plain, aligns)
        }
        Event::Begin(Grouping::Matrix { alignment }) => {
            (MatrixKind::Plain, vec![map_align(*alignment)])
        }
        Event::Begin(Grouping::SubArray { alignment }) => {
            (MatrixKind::Plain, vec![map_align(*alignment)])
        }
        Event::Begin(Grouping::Cases { .. }) => (MatrixKind::Cases, vec![Align::Left, Align::Left]),
        Event::Begin(
            Grouping::Aligned
            | Grouping::Align { .. }
            | Grouping::Alignat { .. }
            | Grouping::Alignedat { .. }
            | Grouping::Split,
        ) => (MatrixKind::Aligned, vec![Align::Right, Align::Left]),
        // Gather(ed): single centered column.
        _ => (MatrixKind::Plain, vec![Align::Center]),
    };
    *i += 1; // consume the Begin
    ...
    loop {
        match events.get(*i) {
            ...
            // `\hline` rows carry no cells of their own; skip (no rule support yet).
            Some(Event::EnvironmentFlow(EnvironmentFlow::StartLines { .. })) => {
                *i += 1;
            }
    ...
    // Drop a wholly empty trailing row left by `… \\ \end`.
    if rows.last().is_some_and(|r| r.iter().all(|c| c.is_empty())) {
        rows.pop();
    }

    MathNode::Matrix { rows, col_align, kind }
}
...
pub fn layout(src: &str, opts: &MathOptions) -> Option<(Box, Face<'static>)> {
...
fn layout_node(ctx: &Ctx, node: &MathNode, style: Style, cramped: bool) -> Option<Box> {
    match node {
        ...
        MathNode::Matrix { rows, col_align, kind } => {
            layout_matrix(ctx, rows, col_align, *kind, style)
        }
...
fn layout_matrix(
    ...
    // bodies are text style (TeX `\textstyle`).
    ...
    let arraystretch = 1.0; // default \arraystretch
    ...
    // right|left pair (even→odd) which touches so the `&` boundary lines up.
