...
/// `\substack{ROWS}` → `{\begin{matrix}ROWS\end{matrix}}` — the `ROWS` already use
/// `\\` row separators that `matrix` understands, and the wrapping `{…}` keeps it a
/// single element under operator limits (`\sum_{\substack{…}}`). Leaves the form
/// verbatim if no braced argument follows.
///
/// Style caveat: real TeX sets `\substack` content at a smaller (script) size; we
/// emit the matrix at the current limit-style instead. That style downgrade is a
/// TODO — the stacked rows render correctly, just one step larger than LaTeX.
fn parse_substack(src: &str, pos: usize) -> Option<(String, usize)> {
    ...
    Some((format!(r"{{\begin{{matrix}}{rows}\end{{matrix}}}}"), after))
}
