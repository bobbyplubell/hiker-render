...
//!
//! Not implemented (out of subset): `sparse`/`uneven` edge-skipping (a rarely
//! used cosmetic mode for odd-width sparse registers).
...
pub fn render(
    ...
    let bits = opt_int(root, "bits").filter(|b| *b >= 1).map(|b| b as u32).unwrap_or(total_bits);
