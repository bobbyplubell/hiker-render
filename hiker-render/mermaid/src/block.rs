...
//! as top-level blocks (no sub-grid nesting). Also skipped: `classDef`/`class`/
//! `style` styling, `<[ ]>` block-arrows, and arrow directions beyond the basic
//! `-->` / `--` / `---` forms (all treated as a simple directed/undirected edge).
//!
...
/// One declared block: an id, its label (defaults to the id), shape, and how
...
fn parse_block(src: &str) -> Result<BlockDiagram, String> {
    ...
    for raw in src.lines() {
        ...
        // Skip styling / class / nesting tokens we don't render.
        let first = line.split_whitespace().next().unwrap_or("");
        if matches!(first, "classDef" | "class" | "style" | "end") {
            continue;
        }

        parse_statement_line(line, &mut blocks, &mut edges, &mut space_counter);
    }
...
fn draw_svg(layout: &Layout, diagram: &BlockDiagram, opts: &MermaidOptions) -> String {
    ...
    let fill_op = opacity_attr("fill-opacity", opts.node_fill);
    let stroke = rgb(opts.node_stroke);
    let stroke_op = opacity_attr("stroke-opacity", opts.node_stroke);
    ...
    for p in &layout.placed {
        ...
        match p.block.shape {
            Shape::Rect => {
                let _ = write!(
                    s,
                    "<rect x=\"{:.2}\" y=\"{:.2}\" width=\"{:.2}\" height=\"{:.2}\" fill=\"{fill}\"{fill_op} stroke=\"{stroke}\"{stroke_op}/>",
                    p.x, p.y, p.w, p.h
                );
            }
            Shape::Round => {
                ...
                let _ = write!(
                    s,
                    "<rect x=\"{:.2}\" y=\"{:.2}\" width=\"{:.2}\" height=\"{:.2}\" rx=\"{r:.2}\" ry=\"{r:.2}\" fill=\"{fill}\"{fill_op} stroke=\"{stroke}\"{stroke_op}/>",
                    p.x, p.y, p.w, p.h
                );
            }
            Shape::Diamond => {
                ...
                let _ = write!(
                    s,
                    "<polygon points=\"{:.2},{:.2} {:.2},{:.2} {:.2},{:.2} {:.2},{:.2}\" fill=\"{fill}\"{fill_op} stroke=\"{stroke}\"{stroke_op}/>",
        ...
        if !p.block.label.is_empty() {
            ...
            let _ = write!(
                s,
                "<text x=\"{cx:.2}\" y=\"{cy:.2}\" font-family=\"{ff}\" font-size=\"{fs:.2}\" fill=\"{text_col}\" text-anchor=\"middle\" dominant-baseline=\"central\">{label}</text>",
    ...
    for e in &diagram.edges {
        ...
        if !e.label.is_empty() {
            ...
            // Small background so the label is readable over the line.
